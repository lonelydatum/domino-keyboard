...
function start() {
	...
	(0, _commonJsCommonJs.setEndFrameLandscape)(bgHeight);

	// domInit(105, -1)

	(0, _commonJsCommonJs.frame)(".frame1");

	tl.add((0, _commonJsDomJs.video)(17, .05), "+=1.5");

	tl.to(['.frame1', '.frameEnd_a'], speed, { x: '-=' + w, ease: ease }, "+=3");

	(0, _commonJsCommonJs.frame)(".frameEnd_a");
	(0, _commonJsCommonJs.frame)(".frameEnd_b");

	tl.set(".logo", { opacity: 1 });
	tl.to([".frameEnd_a", ".frameEnd_b"], 1.2, { x: '-=' + (w + bgHeight), ease: ease }, "+=4");
}
